...
test("MdEditor preserves save and navigation guards while loading WYSIWYG client-only",async()=>{const source=await readFile(new URL("../src/components/md/MdEditor.tsx",import.meta.url),"utf8");assert.match(source,/const changed=content!==initialContent/);assert.match(source,/disabled=\{!changed\|\|saving\}/);assert.match(source,/beforeunload/);assert.match(source,/ssr:false/);assert.match(source,/WysiwygEditor/)});
