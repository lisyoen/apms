...
before(async () => {
  ...
  const login = await fetch(`${base}/api/auth/login`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email: env.DIRIGO_ADMIN_EMAIL, password: env.DIRIGO_ADMIN_PASSWORD }) });
  ...
  const storageSlug = (await client.query("SELECT slug FROM users WHERE lower(email)=lower($1)", [env.DIRIGO_ADMIN_EMAIL])).rows[0].slug;
  ...
  reportsDirectory = path.join(env.DIRIGO_DATA_ROOT, storageSlug, slug, "tasks", "reports");
