...
인증은 기본값이 고정 24시간인 서명 JWT를 사용한다. `dirigo_session` 쿠키에는 항상 같은 기간의 `Max-Age`와 `Expires`를 부여하므로 브라우저를 닫았다 열어도 만료 시각까지 로그인이 유지되고, 만료 후에는 로그인 화면으로 이동한다. 쿠키의 `HttpOnly`, `Secure`, `SameSite=Lax` 속성은 유지한다. remember 세션 분기나 장기 refresh token은 없다.

로그인 화면의 **ID/PW 저장** 체크박스는 브라우저 전용이며 인증 API로 전송하지 않는다. 체크한 로그인에 성공하면 이메일과 Base64로 난독화한 비밀번호를 `localStorage`에 저장하고, 이후 로그인 화면 방문 시 두 입력값과 체크 상태를 복원하되 자동 제출하지 않는다. 체크하지 않은 로그인에 성공하면 두 값을 모두 삭제한다. Base64는 암호화가 아니므로 화면에서 공용 PC 사용을 경고한다. 로그아웃은 세션 쿠키만 삭제하고 저장된 자격 증명은 유지한다. IP+이메일 기준 15분 동안 5회 실패 잠금도 그대로 적용한다. 불변 unique 사용자 slug, 워커 환경 격리, 설정 workdir 제한은 구현되었고 write-only 시크릿 저장소와 #009 실행별 LLM 자격 주입은 계획 상태다.
...
데스크톱 프로젝트 워크스페이스는 페이지 overflow를 막은 뷰포트 높이의 2열 컨테이너다. 프로젝트 내비게이션, 주 콘텐츠, 채팅 열은 서로 독립된 스크롤 경계를 가지므로 긴 작업지시서·실행 로그·리포트를 열어도 채팅 패널의 상단 위치와 높이가 바뀌지 않는다. 작업 목록은 고정된 요약·새 작업 툴바 아래에서 스크롤하고, 열린 작업은 `min-height: 0`인 전용 flex 자식에서 내부 스크롤한다. 768px 이하에서는 기존 세로 스택과 페이지 스크롤을 위해 이 제약을 해제한다. 문서 탭은 기존 편집기 높이 계산과 주 콘텐츠 스크롤을 그대로 사용한다.
...
상단 헤더는 고정하고 메시지 목록만 스크롤한다. 하단 80px 이내일 때만 새 내용을 자동으로 따라가며, 그 밖에서는 미확인 배지가 있는 최신 메시지 이동 버튼을 표시한다. 사용자 메시지와 완료된 assistant 메시지는 저장된 Markdown 원문 `content`를 복사하며 구형 클립보드 폴백을 제공한다.
