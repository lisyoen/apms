...
인증은 갱신 가능한 12시간 JWT와 IP+이메일 기준 15분 동안 5회 실패 잠금을 적용한다. 불변 unique 사용자 slug, 워커 환경 격리, 설정 workdir 제한은 구현되었고 write-only 시크릿 저장소와 #009 실행별 LLM 자격 주입은 계획 상태다.
...
데스크톱 프로젝트 화면은 왼쪽 경계에 6px 드래그 핸들이 있는 기본 420px 패널을 사용한다. 폭은 320px부터 화면의 70%까지 제한되고 `apms.chat.panel.width`로 모든 프로젝트에 공유되며, 더블클릭하면 420px로 복원된다. 전체화면 선호는 `apms.chat.panel.fullscreen`에 저장하고 화면 폭이 900px 이하이면 항상 전체화면을 사용한다.
...
프로젝트 지침·핸드오버·설정뿐 아니라 이후의 공통 지침과 drafts 등 모든 Markdown 편집 화면은 `MdEditor`를 재사용한다. 이 컴포넌트가 분할 미리보기, 키보드 들여쓰기·저장, 변경 상태 이탈 방지, 충돌 선택을 담당한다. 각 화면은 문서 로드와 `If-Match` 저장 콜백을 제공하고 성공 뒤 `MdViewer`로 복귀시켜 저장된 내용으로 제목과 목차를 다시 생성해야 한다. 기능별 textarea 편집기를 별도로 만들지 않는다.
# LLM 헬스와 채팅 시간 흐름
