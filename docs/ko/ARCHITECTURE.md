...
공용 헤더 계정명은 DB의 표시 이름을 우선 사용하고 없으면 이메일을 표시하며, 로그인 필수 `/settings`로 연결한다. 최초 개인설정 화면은 이메일, 불변 slug, 역할, 표시 이름을 읽기 전용으로 보여 준다. 편집, 비밀번호, 환경설정, 알림 제어는 #033으로 미룬다. 루트 metadata에서 모바일 전화번호·이메일 형식 자동 감지를 차단하므로 계정명이 운영체제 이메일 앱 진입점으로 바뀌지 않는다.
