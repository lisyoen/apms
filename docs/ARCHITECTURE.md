...
On desktop, the project workspace is a viewport-height two-column container with page overflow disabled. The project navigation, main content, and chat column keep independent scroll boundaries, so opening a long task instruction, execution log, or report cannot change the chat panel's top position or height. The task list scrolls below its sticky summary/new-task toolbar, while an open task uses a dedicated flex child with `min-height: 0` and internal scrolling. At 768 px and below, these constraints are removed for the existing vertically stacked, page-scrolling layout. Document tabs continue to use the existing editor sizing and the main content scroller.
