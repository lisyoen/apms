...
export async function currentUser(){const session=await readSession((await cookies()).get("apms_session")?.value);if(!session)return null;const r=await db.query("SELECT id,email,display_name,role,disabled_at FROM users WHERE lower(email)=lower($1)",[session.email]);return r.rows[0]??null;}
