...
export async function currentUser(){const session=await readSession((await cookies()).get("dirigo_session")?.value);if(!session)return null;const r=await db.query("SELECT id,email,slug,display_name,role,disabled_at FROM users WHERE lower(email)=lower($1) AND disabled_at IS NULL",[session.email]);return r.rows[0]??null;}
