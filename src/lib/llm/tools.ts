...
import { contentHash,createTaskFile,initializeProject,readDocument,slugify,userSlug,writeDocument,type DocKind,type Frontmatter } from "@/lib/storage";
export const toolSpecs=[
 ...
 {name:"read_doc",description:"프로젝트 문서를 읽습니다.",parameters:{type:"object",properties:{project:{type:"string"},kind:{type:"string"}},required:["project","kind"]}},
 {name:"update_doc",description:"프로젝트 문서를 갱신합니다.",parameters:{type:"object",properties:{project:{type:"string"},kind:{type:"string"},content:{type:"string"}},required:["project","kind","content"]}}
];
...
export async function runTool(user:{id:string,email:string},sessionId:string,name:string,args:any){
 ...
 if(name==="read_doc")return{kind:args.kind,content:await readDocument(userSlug(user.email,user.id),p.slug,String(args.kind) as DocKind)};
 if(name==="update_doc"){await writeDocument(userSlug(user.email,user.id),p.slug,String(args.kind) as DocKind,String(args.content));return{updated:true,kind:args.kind};}
