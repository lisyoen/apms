...
import { contentHash,createTaskFile,initializeProject,readDocument,slugify,writeDocument,type DocKind,type Frontmatter } from "@/lib/storage";
...
export async function runTool(user:{id:string,email:string,slug:string},sessionId:string,name:string,args:any){
 ...
 if(name==="read_doc")return{kind:args.kind,content:await readDocument(user.slug,p.slug,String(args.kind) as DocKind)};
 if(name==="update_doc"){await writeDocument(user.slug,p.slug,String(args.kind) as DocKind,String(args.content));return{updated:true,kind:args.kind};}
