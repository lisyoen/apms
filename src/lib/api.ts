...
export async function currentUser() { const session = await readSession((await cookies()).get("dirigo_session")?.value); if (!session) return null; const result = await db.query("SELECT id,email,slug,role FROM users WHERE lower(email)=lower($1) AND disabled_at IS NULL", [session.email]); const row = result.rows[0]; return row ? { ...row, storageSlug: row.slug } : null; }
